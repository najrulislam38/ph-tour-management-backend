...
//   // const filterData = await Tour.find(searchQuery)
//   //   .find(filter)
//   //   .sort(sort)
//   //   .select(fields)
//   //   .skip(skip)
...
//     .skip(skip)
//     .limit(limit);
//   // const filterData = await Tour.find({
//   //   $or: searchArray,
//   // });
//   // const filterData = await Tour.find({
//   //   $or: [
...
const updateTour = async (id: string, payload: Partial<ITour>) => {
  ...
  if (
    ...
    const resetDBImages = existingTour.images?.filter(
...
const getSingleTourType = async (name: string) => {
  return await TourType.findOne({ name });
};
